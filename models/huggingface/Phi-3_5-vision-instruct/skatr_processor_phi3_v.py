...
class Phi3VImageProcessorSkatr(BaseImageProcessor):
    r"""
    ...
    def preprocess(
        ...
        # if is_numpy_array(images[0]):
        #     images = [torch.from_numpy(img) for img in images]
        # elif not is_torch_tensor(images):
        #     raise ValueError(
        #         "Invalid image type. Must be of type numpy.ndarray or torch.Tensor."
        #     )
...
AutoImageProcessor.register("Phi3VImageProcessorSkatr", Phi3VImageProcessorSkatr)

transformers.Phi3VImageProcessorSkatr = Phi3VImageProcessorSkatr

class Phi3VProcessorSkatr(ProcessorMixin):
    r"""
