...
class NdViT(nn.Module):
    """
    An N-d vision transformer network.
    """
    ...
    def __init__(self, cfg):
        ...
        # position encoding
        self.init_pos_encoding(axis_sizes)
    ...
    def init_pos_encoding(self, axis_sizes):
        fourier_dim = self.dim // (
            2 * self.image_dim
        )  # sin/cos features for each image dim
        w = torch.arange(fourier_dim) / (fourier_dim - 1)
        w = (1.0 / (10_000**w)).repeat(self.image_dim)
        self.pos_encoding_freqs = nn.Parameter(
            w.log() if self.cfg.learn_pos_encoding else w,
            requires_grad=self.cfg.learn_pos_encoding,
        )
    ...
    def eval_pos_encoding(self, scales=None):
        ...
        if self.cfg.learn_pos_encoding:
            freqs = self.pos_encoding_freqs.exp().chunk(self.image_dim)
        else:
            freqs = self.pos_encoding_freqs.chunk(self.image_dim)
    ...
    def forward(self, x, mask=None, scales=None):
        ...
        if self.cfg.use_patching:
            ...
            if self.cfg.use_mask_token:
                ...
                x = x + self.eval_pos_encoding(scales=scales)
            else:
    ...
    def to_patches(self, x):
        match self.image_dim:
            case 2:
                x = rearrange(
                    x,
                    "b c (x p1) (y p2)-> b (x y) (p1 p2 c)",
    ...
    def from_patches(self, x):
        match self.image_dim:
            case 2:
                x = rearrange(
                    x,
                    "b (x y) (p1 p2 c) -> b c (x p1) (y p2)",
                    **dict(
                        zip(
                            ("x", "y", "p1", "p2"),
                            self.num_patches + self.cfg.patch_shape,
                        )
                    ),
                )
            case 3:
                x = rearrange(
                    x,
                    "b (x y z) (p1 p2 c) -> b c (x p1) (y p2) (z p3)",
                    **dict(
                        zip(
                            ("x", "y", "z", "p1", "p2", "p3"),
                            self.num_patches + self.cfg.patch_shape,
...
def check_shapes(cfg):
    ...
    assert (
        not cfg.hidden_dim % 4
    ), f"Hidden dim should be divisible by 4 (for fourier position embeddings)"
...
class NdPredictorViT(NdViT):
    ...
    def forward(self, ctx, ctx_mask, tgt_mask):
        ...
        pos_encoding = repeat(self.eval_pos_encoding(), "t d -> b t d", b=B)
