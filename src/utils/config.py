...
# def update_config_from_prev(cfg, hydra_cfg, prev_exp_dir):
#     prev_overrides = OmegaConf.from_dotlist(OmegaConf.to_object(get_prev_overrides(prev_exp_dir)))
#     overrides = OmegaConf.from_dotlist(OmegaConf.to_object(hydra_cfg.overrides.task))

#     print(prev_overrides)
#     print(overrides)
#     # TODO: Using cfg could let outdated runs be loaded without having to edit their cfg.
#     # with open_dict(cfg):
#     # return cfg
#     return OmegaConf.merge(cfg, prev_overrides, overrides)
...
def check_cfg(cfg, log):
    ...
    if cfg.num_cpus and not cfg.data.file_by_file:
        log.warn(
            f"Using {cfg.num_cpus} cpus not reading from disk. Training may be slower."
        )
